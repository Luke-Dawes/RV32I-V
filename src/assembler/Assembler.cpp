...
std::vector<uint32_t> Assembler::parse() { //this needs a symbol table for like labels for the jump instructions
	...
	for (const std::string& line : text) { //loop through the vector 
		...
		switch (type) {
			...
			case Format::IS_TYPE: {
				...
				if (rs1_it == register_to_number.end() || rd_it == register_to_number.end()) {
					continue;
				}
			...
			case Format::J_TYPE: {
				...
				instruction |= (((immediate_value >> 12) & 0xFF) << 12);
			...
			default:
				std::cout << "unknown or system type" << line << "\n";
				break;
		}
	}
	...
	std::cout << "assembler final code finished \n\n";
